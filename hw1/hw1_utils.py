...
def load_reg_data():
    ...
    noise = torch.normal(0, 0.4, size=X.shape)
    w = 0.5
    b = 1.0
    Y = w * X**2 + b + noise
...
def load_logistic_data():
    ...
    return linear_problem(torch.tensor([-1.0, 2.0]), margin=1.5, size=200)
...
def contour_plot(xmin, xmax, ymin, ymax, pred_fxn, ngrid=33):
    ...
    C = plt.contour(xx, yy, zz, cmap="coolwarm")
    plt.clabel(C)
    plt.show()
...
def linear_problem(w, margin, size, bounds=[-5.0, 5.0], trans=0.0):
    def in_margin(x):
        return torch.abs(w.flatten().dot(x.flatten())) / torch.norm(w) < margin

    def half_margin(x):
        return (
            0.6 * margin < w.flatten().dot(x.flatten()) / torch.norm(w) < 0.65 * margin
        )
